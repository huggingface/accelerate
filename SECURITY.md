...
Exact released version or commit SHA you reproduced on (e.g. v1.14.0 / a1b2c3d).
Not "latest" or "main".

### Affected component
The public API, module, or entry point involved (e.g. `load_checkpoint_and_dispatch`).
...
**CLI and developer utilities are operator tools.** Scripts you run yourself against inputs you chose (the `accelerate config` / `accelerate launch` / `accelerate estimate-memory` CLIs, utility scripts, dev helpers) are not a library API attack surface. Behaviors such as a script deserializing a file you pointed it at are within the operator's trust.
...
- Issues that require loading an untrusted artifact and amount to the documented load-time risk above (code execution / file access on load of a malicious model, checkpoint, config, or pickle).
- Findings in `examples/`, `benchmarks/`, documentation, tests, or other non-packaged reference material. These are out of scope for private security advisories (GHSA/CVE), but valid security issues should still be reported through the normal issue/PR process.
...
- The distributed launcher and the rendezvous/communication endpoints it wires up (`accelerate launch`, `torchrun`, elastic rendezvous, NCCL/Gloo stores) are training-cluster infrastructure you deploy and network-isolate yourself, not a hardened public service. Their reachability or availability on a network you chose to expose is a deployment concern, not a library vulnerability — absent a genuine cross-tenant or isolation break.
