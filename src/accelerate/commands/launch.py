...
def launch_command(args):
    # Sanity checks
    if sum([args.multi_gpu, args.multi_cpu, args.tpu, args.mps, args.use_deepspeed, args.use_fsdp]) > 1:
        raise ValueError(
            "You can only use one of `--multi_cpu`, `--multi_gpu`, `--mps`, `--tpu`, `--use_deepspeed`, `--use_fsdp` at a time."
        )
