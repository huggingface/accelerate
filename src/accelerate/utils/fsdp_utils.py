...
def _prepare_sd_options(fsdp_plugin):
    ...
    if fsdp_plugin.fsdp_version == 2:
        ...
        cpu_offload |= getattr(fsdp_plugin.state_dict_config, "cpu_ram_efficient_loading", False)
...
def fsdp2_prepare_model(accelerator, model: torch.nn.Module) -> torch.nn.Module:
    ...
    # Set it to None if it doesn't exist and do the upcast if mixed precision is not enabled
