...
def convert_model(model, to_transformer_engine=True, _convert_linear=True, _convert_ln=True):
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.Linear) and to_transformer_engine and _convert_linear:
            ...
            te_module = te.Linear(module.in_features, module.out_features, bias=has_bias, dtype=module.weight.dtype)
            ...
            te_module = te.LayerNorm(module.normalized_shape[0], eps=module.eps, dtype=module.weight.dtype)
            te_module.weight.data = module.weight.data.clone()
            te_module.bias.data = module.bias.data.clone()
            ...
            has_bias = module.bias is not None
            new_module = nn.Linear(module.in_features, module.out_features, bias=has_bias, dtype=module.weight.dtype)
            ...
            new_module = nn.LayerNorm(module.normalized_shape[0], eps=module.eps, dtype=module.weight.dtype)
            new_module.weight.data = module.weight.data.clone()
            new_module.bias.data = module.bias.data.clone()
