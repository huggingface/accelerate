...
class PrefixedDataset(Mapping):
    ...
    def __iter__(self):
        # Yield the keys without the prefix so they can be passed back to `__getitem__`
        return iter([key[len(self.prefix) :] for key in self.dataset if key.startswith(self.prefix)])
