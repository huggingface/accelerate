...
def fallback_allocate(
    ...
    Returns:
        `Tuple[Optional[str], Optional[nn.Module], List[Tuple[str, nn.Module]]]`:
        The name of the module that fits in the
    """
    ...
    while modules_to_search:
        ...
        if module_sizes[name] <= size_limit:
            ...
            for tied_param in tied_params:
                tied_module_indices = [i for i, (n, _) in enumerate(modules_to_search) if n in tied_param]
                if tied_module_indices:
                    tied_module_names.append(modules_to_search[tied_module_indices[0]][0])

            module_size_with_ties = module_sizes[name]
