    ...
    with torch.no_grad():
        ...
        if is_buffer:
            ...
            if param_cls.__name__ in ["Int8Params", "FP4Params", "Params4bit"]:
                ...
                new_value = torch.nn.Parameter(param_cls(new_value.layout_tensor, new_value.block_size, new_value.shape, new_value.quant_min, new_value.quant_max, new_value.zero_point_domain), requires_grad=old_value.requires_grad).to(device)
            else:
