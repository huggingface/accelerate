...
def compile_regions(module: torch.nn.Module, **compile_kwargs) -> torch.nn.Module:
    ...
    them sequentially to hit the compiler's cache. For example, in `GPT2LMHeadModel`, the repeated block is
    `GPT2Block`, and can be accessed as `model.transformer.h[0]`.
