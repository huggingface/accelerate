...
def concatenate(data, dim=0):
    """
    Recursively concatenate the tensors in a nested list/tuple/dictionary of lists of tensors, or str with the same shape.
    ...
    if isinstance(data[0], list) and all(isinstance(x, str) for x in data[0]):
        return honor_type(data, [item for sublist in data for item in sublist])
