...
def compare_against_test(base_filename: str, full_filename: str, feature_filename: str):
    """
    ...
    base_file_training_func = clean_lines(get_train_func(base_file_contents))
    full_file_training_func = clean_lines(get_train_func(full_file_contents))
    feature_file_contents = clean_lines(get_train_func(feature_file_contents))
    ...
    new_feature_code = [
        line for line in feature_file_contents if (line not in base_file_training_func) and (line.lstrip() != _dl_line)
    ]

    # Extract out just the new parts from the full_file_training_func
    new_full_example_parts = [
        line
        for line in full_file_training_func
        if (line not in base_file_training_func) and (line.lstrip() != _dl_line)
    ]
