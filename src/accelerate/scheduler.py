    ...
    def step(self, *args, **kwargs):
        ...
        for opt in self.optimizers:
            if opt.step_was_skipped:
                print(
                    f"Skipped scheduler step because the optimizer step was skipped on worker {AcceleratorState().process_index}"
                )
                return
        if self.split_batches:
            ...
            print(
                f"Stepped scheduler {num_processes} times because of `split_batches=False` on worker {AcceleratorState().process_index}"
            )
