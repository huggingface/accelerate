...
If you already shard the dataloader yourself (for example with a rank-aware `DistributedSampler` or a pre-sliced
iterable), set `already_sharded=True` on [`~utils.DataLoaderConfiguration`]. Accelerate still wraps the loader in
[`~data_loader.DataLoaderShard`] for device placement, `set_epoch`, and state tracking, but does not apply
`BatchSamplerShard`, `IterableDatasetShard`, or a second Hugging Face `shard()`. This is incompatible with
