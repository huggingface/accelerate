...
For this tutorial we will be utilizing both the [`Accelerator`] object and the [`PartialState`] object. The former is
is the high-level API that you will use most of the time when using this library, the latter is the low-level API that
you can use to defer the execution of some instructions to specific processes directly *without* the need to create an 
[`Accelerator`] object first.

To import and instantiate either do:
...
These two objects will be referenced as such throughout the rest of this tutorial.

<Tip>

    Every function that is shown from `PartialState` in this tutorial can be found in the `Accelerator` class as well.
...
```py
patial_state.wait_for_everyone()
```
...
<Tip>
    ...
    Some of these are utilized with the [`~PartialState.main_process_first`] context manager, which utilizes [`~PartialState.wait_for_everyone`] to 
...
if not partial_state.is_main_process:
    datasets = load_dataset("glue", "mrpc")
else:
    partial_state.wait_for_everyone()
```
