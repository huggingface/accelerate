...
<Tip>
When using the `notebook_launcher` on a TPU, if `accelerate config` was configured with `bf16` or if `bf16` is passed into the `mixed_precision` parameter, both are automatically utilized. 
</Tip>
