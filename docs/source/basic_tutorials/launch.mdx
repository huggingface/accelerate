...
If you only want to use the default configuration saved from `accelerate config`, the custom config file path would not need to be used.
Why is it useful to the point you should **always** run `accelerate config`? Remember that earlier call to `accelerate launch` as well as `torchrun`?
Post configuration, to run that script with the needed parts you just need to use `accelerate launch` outright:
