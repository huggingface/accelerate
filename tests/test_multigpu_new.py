...
from accelerate.test_utils import device_count, require_huggingface_suite
...
def do_train(generator, train_dl, model, optimizer, accelerator, old_model):
    set_seed(42)
...
# class CoreTester(MultiThreadedTestCase):
#     @property
#     def world_size(self):
#         return device_count
...
#         state = PartialState()
#         assert state.num_processes == device_count
           
#     def test_process_execution(self):
#         accelerator = Accelerator()
#         num_processes = accelerator.num_processes
#         assert num_processes == 2, num_processes
...
#     def test_split_between_processes_list(self):
#         state = PartialState()
...
#     def test_split_between_processes_nested_dict(self):
#         state = PartialState()
#         a = [1, 2, 3, 4, 5, 6, 7, 8]
#         b = ["a", "b", "c", "d", "e", "f", "g", "h"]
...
#         state.wait_for_everyone()
#     def test_split_between_processes_tensor(self):
#         state = PartialState()
...
#     def test_split_between_processes_evenly(self):
#         state = PartialState()
...
#     def test_split_between_processes_dataset(self):
#         state = PartialState()
#         data = datasets.Dataset.from_list([dict(k=v) for v in range(2 * state.num_processes)])
#         with state.split_between_processes(data, apply_padding=False) as results:
#             assert (
#                 len(results) == 2
#             ), f"Each process did not have two items. Process index: {state.process_index}; Length: {len(results)}"

#         data = datasets.Dataset.from_list([dict(k=v) for v in range(2 * state.num_processes - 1)])
#         with state.split_between_processes(data, apply_padding=False) as results:
#             if state.is_last_process:
#                 assert (
#                     len(results) == 1
...
#     def test_rng_syncronization(self):
#         state = PartialState()
...
#     def test_reinstantiated_state(self):
...
#         assert "`AcceleratorState` object has no attribute" in str(cm.exception.args[0])
#         assert "This happens if `AcceleratorState._reset_state()`" in str(cm.exception.args[0])

# class DataLoaderTester(MultiThreadedTestCase):
#     @property
#     def world_size(self):
#         return device_count

#     def test_dataloader_preparation_non_shuffled(self):
#         state = PartialState()
...
#     def test_dataloader_preparation_shuffled(self):
#         state = PartialState()
#         length = 32 * state.num_processes
...
#     def test_central_dataloader_preparation(self): 
#         state = PartialState()
#         length = 32 * state.num_processes
...
#     def test_seedable_sampler(self):
#         set_seed(42)
...
#     def test_seedable_sampler_batch_sampler_shard(self):
#         set_seed(42)
...
class TrainingTester(MultiThreadedTestCase):
    ...
    def perThreadSetUp(self):
        # Needed for setting up the `PartialState` properly
    ...
    def test_distributed_spawning(self):
        state = PartialState()
        # Because we can potentially just skip this silently, prints out `state` as well
        print(state)
